...
import { modulesS2 } from "./moduleS2";
import { styles } from '../../Styles';

export default function Semester2Table({ specialty = "AI" }) {
  ...
  useEffect(() => {
    AsyncStorage.getItem(`${specialty}_S2_NOTES`).then((data) => {
      if (data) {
        try {
  ...
  const saveNotes = (updated) => {
    setSubjects(updated);
    AsyncStorage.setItem(`${specialty}_S2_NOTES`, JSON.stringify(updated));
  };
  ...
  const calcAverages = () => {
    ...
    const groupedByUE = [
      { title: "UE Fondamentale (UEF2)", range: [0, 4] },
      { title: "UE Méthodologique (UEM2)", range: [4, 6] },
      { title: "UE Découverte (UED2)", range: [6, 7] },
      { title: "UE Transversale (UET2)", range: [7, 8] },
    ];
    ...
    AsyncStorage.setItem(`${specialty}_S2_AVERAGE`, moyenneGenerale.toFixed(2));
  };
  ...
  const groupedByUE = [
    { title: "UE Fondamentale (UEF2)", data: (subjects || []).slice(0, 4) },
    { title: "UE Méthodologique (UEM2)", data: (subjects || []).slice(4, 6) },
    { title: "UE Découverte (UED2)", data: (subjects || []).slice(6, 7) },
    { title: "UE Transversale (UET2)", data: (subjects || []).slice(7, 8) },
  ];
